...
@section('content')
    <div class="col-lg-12">
        <h1>Move Raw Materials To Different Storage</h1>
    </div>
    <hr>
    <div class="col-lg-12">
        <h2>{{$RawMats->item_name}}</h2>
        <form method="POST" action="/move/RawMatssave" enctype="multipart/form-data">
            @csrf

            <div class="col-md-6">
                <input id="id" type="text" class="form-control" name="id" hidden value="{{$RawMats->id}}">
            </div>
                        ...
                        <optgroup label="{{$stock->room_name}}">
                            @foreach ($STO as $storage)
                                @if ($stock->id == $storage->stockroom_id)
                                    @if ($RawMats->storage_id == $storage->id)
